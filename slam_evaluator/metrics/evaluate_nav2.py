...
class Nav2Evaluator(Node):
    ...
    def run_evaluation(self):
        ...
        for i, (start, goal) in enumerate(zip(self.start_poses, self.goal_poses)):
            ...
            self.get_logger().info(f"Test {i+1} completed. Success: {success}, Time: {duration:.2f}s, Dist: {initial_distance}, Recov: {number_of_recoveries}")
            
        # Write to CSV
        with open(self.output_csv, mode='w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['test_id', 'success', 'duration', 'initial_distance', 'recoveries', 'result_code'])
            writer.writeheader()
            for r in results:
                writer.writerow(r)
        
        self.get_logger().info(f"Evaluation finished. Results saved to {self.output_csv}")
    ...
    def run_kidnapping_evaluation(self, start, goal):
        ...
        while not self.navigator.isTaskComplete():
            ...
            if not kidnapped and self.navigator.getFeedback() and self.navigator.getFeedback().distance_remaining < 5.0:
                self.get_logger().info("KIDNAPPING ROBOT!")
                wrong_pose = create_pose(self.navigator, start.pose.position.x + 2.0, start.pose.position.y - 2.0, 0)
...
def main():
    ...
    parser.add_argument("--output", default="nav2_metrics.csv", help="Output CSV file")
    args, unknown = parser.parse_known_args()

    rclpy.init()
    
    # We would read these from a file ideally. Hardcoded for now.
    nav = BasicNavigator()
    start_poses = [
        create_pose(nav, 0.0, 0.0, 0.0),
        create_pose(nav, 1.0, 0.0, 0.0)
    ]
    goal_poses = [
        create_pose(nav, 5.0, 0.0, 0.0),
        create_pose(nav, 0.0, 0.0, 3.14)
    ]
    
    evaluator = Nav2Evaluator(start_poses, goal_poses, args.output)

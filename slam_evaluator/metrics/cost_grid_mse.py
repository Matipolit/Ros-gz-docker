...
def compute_cost_grid(graph, width, height, goal_x, goal_y):
    ...
    dist_matrix = dijkstra(
        csgraph=graph, directed=False, indices=goal_id, return_predecessors=False
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute Cost Grid MSE between two occupancy grid maps"
    )
    parser.add_argument("--map1_yaml", required=True, help="Path to first map YAML")
    parser.add_argument("--map2_yaml", required=True, help="Path to second map YAML")
    parser.add_argument(
        "--goal_x", type=float, default=0.0, help="Goal X in world coordinates"
    )
    parser.add_argument(
        "--goal_y", type=float, default=0.0, help="Goal Y in world coordinates"
    )

    args = parser.parse_args()
    evaluate_mse(args.map1_yaml, args.map2_yaml, args.goal_x, args.goal_y)

...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate ATE and RPE between trajectories using Horn's SE(3) alignment."
    )
    parser.add_argument(
        "--gt_trajectory",
        required=True,
        type=Path,
        help="Ground truth trajectory file (CSV/TXT)",
    )
    parser.add_argument(
        "--est_trajectories",
        required=True,
        nargs="+",
        type=Path,
        help="Estimated trajectory file(s) to compare",
    )
    parser.add_argument(
        "--output_csv",
        required=True,
        type=Path,
        help="Output CSV file for the metrics report",
    )
    args = parser.parse_args()

    t_gt, p_gt, q_gt = parse_trajectory(args.gt_trajectory)
    if len(t_gt) == 0:
        print(
            f"Error: No valid data found in Ground Truth trajectory {args.gt_trajectory}"
        )
        sys.exit(1)

    results = []

    for est_path in args.est_trajectories:
        ...
        p_gt_m, p_est_m, q_gt_m, q_est_m = associate_trajectories(
            t_gt, p_gt, q_gt, t_est, p_est, q_est
        )
        ...
        rpe_mean, rpe_median, rpe_perc = compute_rpe(
            p_gt_m, q_gt_m, p_est_aligned, q_est_aligned, delta=1
        )
    ...
    print(f"Metrics successfully written to {args.output_csv}")

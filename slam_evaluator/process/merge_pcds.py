...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Merge .pcd files from a folder into one point cloud."
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if not source_folder.is_dir():
        print(
            f"Error: Source folder does not exist or is not a directory: {source_folder}",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        f"Saved merged point cloud with {len(merged.points)} points to: {output_file}"
    )
    return 0

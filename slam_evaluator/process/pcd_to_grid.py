...
def pcd_to_grid(
    ...
    grid = np.full(
        (height, width), 254, dtype=np.uint8
    )  # assume white/free where no obstacle, maybe better to trace rays for unknown?
    ...
    # Threshold
    grid[occupancy_counts >= occupancy_threshold] = 0  # Occupied
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert PCD/PLY to 2D Occupancy Grid (PGM+YAML)"
    )
    parser.add_argument("--input", required=True, help="Input PCD or PLY file")
    parser.add_argument(
        "--output_prefix",
        required=True,
        help="Output prefix (e.g. 'map' to produce map.pgm and map.yaml)",
    )
    parser.add_argument(
        "--resolution", type=float, default=0.05, help="Resolution (m/pixel)"
    )
    parser.add_argument(
        "--min_z", type=float, default=0.1, help="Minimum Z for obstacle"
    )
    parser.add_argument(
        "--max_z", type=float, default=1.5, help="Maximum Z for obstacle"
    )
    parser.add_argument(
        "--occupancy_threshold",
        type=int,
        default=1,
        help="Min points per cell to be occupied",
    )
